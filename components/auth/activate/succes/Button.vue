<template>
    <div @click="passSucces()">
        <NuxtLink to="/login" class="auth_register_succes_button">
            Все понятно
        </NuxtLink>
    </div>
</template>
...
export default {
     methods: {
        passSucces() {
            this.$store.commit("login/resetSuccesEmail", false)
            ...
            this.$store.commit("login/changeAuthTab", 'signin')
            this.$store.commit("login/updateEmail", '') 
        }
    }
}
</script>
...
.auth_icon_wrapper:active {
    color: black;
}

.auth_icon_wrapper:visited {
    color: black;
}
.auth_icon {
    display: grid;
